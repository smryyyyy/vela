...
【具体生成要求】
- **体量硬性要求：必须严格达到 {{word_number}} 字（允许误差±5%）。字数不足将被判定为不合格！** 需要在已有内容基础上充分扩展：增加思思的内心独白（她的双重心理——说出口的话和真实想法）、感官细节描写（视觉/触觉/听觉/嗅觉/味觉）、环境氛围渲染、对话交锋的节奏延展。每个性爱场景必须写清部位、角度、力度、节奏、体温、体液、呼吸、表情——这些细节描写本身就是字数来源，不是注水。
